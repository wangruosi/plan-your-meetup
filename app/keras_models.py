...
def build_DNN(embedding_matrix, max_sequence_length=500,
              topic_num=5, model_type=None, drop_out=0.2):
    ...
    if model_type == 'MLP':
        model = _add_MLP(model, drop_out=drop_out)
    if model_type == 'GRU':
        model = _add_GRU(model, drop_out=drop_out)
    if model_type == 'CNN':
        model = _add_CNN(model, drop_out=drop_out)
    if model_type == "CNN_GRU":
        model = _add_CNN_GRU(model, drop_out=drop_out)
...
def _add_MLP(model, layer_num=2, drop_out=0.2):
    model.add(Flatten())
...
def _add_CNN(model, layer_num=2, drop_out=0.2):
...
def _add_CNN_GRU(model, layer_num=[1, 1], drop_out=0.2):
